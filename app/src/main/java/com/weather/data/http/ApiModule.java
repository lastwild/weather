...
public interface ApiModule {
  ...
  @GET(Constants.Server.GET_FORECAST_FOR_FIVE_DAYS) Call<ForecastForDays> forecastForFiveDays(@Query("id") String city,
      @Query("appid") String apikey);
  ...
  @GET(Constants.Server.GET_HISTORICAL_DATA) Call<ForecastForDays> hourlyHistorical(@Query("q") String city,
      @Query("type") String type, @Query("start") int start, @Query("cnt") int count, @Query("appid") String apiKey);
}
